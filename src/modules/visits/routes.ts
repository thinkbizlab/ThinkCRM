...
import { getCachedResponse, recordResponse } from "../../lib/client-request-log.js";
...
const checkInSchema = z.object({
  ...
  // Mobile-only: stable uuid v4 generated by the offline-sync queue so retries
  // after partial network failures return the cached response instead of
  // re-checking-in. Optional — the web client never sends this.
...
export const visitRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post("/visits/:id/checkin", async (request, reply) => {
    const tenantId = requireTenantId(request);
    ...
    // Mobile idempotency: if this clientRequestId already succeeded, replay
    // the cached response instead of mutating again.
    const clientRequestId = parsed.data.clientRequestId;
    if (clientRequestId) {
      const cached = await getCachedResponse(tenantId, clientRequestId);
      if (cached) {
        return reply.code(cached.responseStatus).send(cached.responseBody);
      }
    }
    ...
    const response = { visit: checkedIn, notifWarnings };
    if (clientRequestId) {
      await recordResponse({
        tenantId,
        clientRequestId,
        userId: repId,
        route: "POST /visits/:id/checkin",
        status: 200,
        body: response as unknown as PrismaRuntime.InputJsonValue
      });
    }
    return response;
  });

  app.post("/visits/:id/checkout", async (request, reply) => {
    const tenantId = requireTenantId(request);
    const repId = requireUserId(request);
    const params = request.params as { id: string };
    ...
    const response = { visit: updated, notifWarnings };
    if (clientRequestId) {
      await recordResponse({
        tenantId,
        clientRequestId,
        userId: repId,
        route: "POST /visits/:id/checkout",
        status: 200,
        body: response as unknown as PrismaRuntime.InputJsonValue
      });
    }
